...
// ---------------------------------------------------------------------------
// Convenience wrapper — used by all the app's list/filter selects.
// Accepts the same value/onChange/options API the pages already use.
// When `searchable` is true, renders a Popover+Command combobox instead of
// the Radix Select (which does not support built-in filtering).
// ---------------------------------------------------------------------------
...
type AppSelectProps<T extends string> = {
  ...
  searchable?: boolean;
  /** Placeholder text for the search input (only used when searchable). */
  searchPlaceholder?: string;
  /** Controlled search value for searchable selects. */
  searchValue?: string;
  /** Controlled search callback for searchable selects. */
  onSearchChange?: (value: string) => void;
  /** Message shown when no options match the search query (only used when searchable). */
  emptyText?: string;
...
function AppSelect<T extends string>({
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            ...
            aria-expanded={open}
            className={cn(
              "flex h-9 w-full items-center justify-between whitespace-nowrap rounded-md border border-input bg-background px-3 py-2 text-xs shadow-sm ring-offset-background focus:outline-none focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50",
              ...
              <CommandGroup>
                {options.map((option) => (
                  <CommandItem
                    ...
                    value={option.label}
                    onSelect={() => {
                      onChange(option.value === value ? ("" as T) : option.value);
                      setOpen(false);
                    }}
                  >
                    <Check
  ...
  const internalValue = value === "" && hasEmptyOption ? EMPTY_SELECT_VALUE : value;

  return (
    <Select
      value={internalValue}
      onValueChange={nextValue => onChange((nextValue === EMPTY_SELECT_VALUE ? "" : nextValue) as T)}
    >
