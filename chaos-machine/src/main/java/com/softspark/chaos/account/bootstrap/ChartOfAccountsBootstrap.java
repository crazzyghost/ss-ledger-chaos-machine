...
public class ChartOfAccountsBootstrap implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        if (bootstrapProperties.accountRoles() == null || bootstrapProperties.accountRoles().isEmpty()) {
        ...
        for (var roleConfig : bootstrapProperties.accountRoles()) {
            ...
            if (roleConfig.defaultVaId() != null && !roleConfig.defaultVaId().isBlank()) {
                ...
                if (existingVa.isEmpty()) {
                    ...
                    vasCreated++;
                    log.debug("Created SYSTEM virtual account {} for role {}",
                            roleConfig.defaultVaId(), roleConfig.role());
                }
            }
        }
        ...
        if (bootstrapProperties.flowSlots() != null) {
            for (var slotConfig : bootstrapProperties.flowSlots()) {
                var existingConfig = flowSlotConfigRepository
                        .findByFlowTypeAndSlotName(slotConfig.flowType(), slotConfig.slotName());
                ...
                if (existingConfig.isPresent()) {
                    ...
                    flowSlotsUpdated++;
                    log.debug("Updated flow slot config: {}.{} -> {}",
                            slotConfig.flowType(), slotConfig.slotName(), slotConfig.accountRole());
                } else {
                    ...
                    flowSlotConfigRepository.save(config);
                    flowSlotsCreated++;
                    log.debug("Created flow slot config: {}.{} -> {}",
                            slotConfig.flowType(), slotConfig.slotName(), slotConfig.accountRole());
                }
            }
        }
        ...
        log.info("Chart of accounts bootstrap completed in {}ms: " +
                        "roles created={}, roles updated={}, VAs created={}, " +
                        "flow slots created={}, flow slots updated={}",
                duration, rolesCreated, rolesUpdated, vasCreated,
                flowSlotsCreated, flowSlotsUpdated);
    }
}
