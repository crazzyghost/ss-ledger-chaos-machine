...
public class FlowConfigService {
  ...
  public List<FlowConfigResponse> getAllFlowConfigs() {
    ...
    Map<FlowType, List<FlowSlotConfig>> configsByFlow =
        flowSlotConfigRepository.findAll().stream()
    ...
    for (var slotUpdate : request.slots()) {
      ...
      if ((slotUpdate.accountRole() == null || slotUpdate.accountRole().isBlank())
        ...
        throw new BadRequestException(
            "Either accountRole or explicitVaId must be provided for slot: "
                + slotUpdate.slotName(),
            null);
      }
      ...
      if (slotUpdate.accountRole() != null && !slotUpdate.accountRole().isBlank()) {
        try {
          ...
          if (!accountRoleRepository.existsById(role)) {
            throw new BadRequestException(
                "Account role not found: " + slotUpdate.accountRole(), null);
          }
        } catch (IllegalArgumentException e) {
          throw new BadRequestException("Invalid account role: " + slotUpdate.accountRole(), null);
        }
      }
      ...
      if (slotUpdate.explicitVaId() != null && !slotUpdate.explicitVaId().isBlank()) {
        if (!virtualAccountRepository.existsById(slotUpdate.explicitVaId())) {
          throw new BadRequestException(
              "Virtual account not found: " + slotUpdate.explicitVaId(), null);
        }
      }

      // Update or create the slot config
      var existingConfig =
          flowSlotConfigRepository.findByFlowTypeAndSlotName(flowType, slotUpdate.slotName());
  ...
  private FlowConfigResponse mapToResponse(FlowType flowType, List<FlowSlotConfig> configs) {
    var slots = configs.stream().map(this::mapSlotToResponse).toList();
