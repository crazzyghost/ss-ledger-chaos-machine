...
public class ChaosEventPublisher {
    ...
    public ChaosEventPublisher(KafkaTemplate<String, Object> kafkaTemplate, MeterRegistry meterRegistry) {
    ...
    public PublishResult publish(String topic, String key, EventEnvelope<?> envelope) {
        ...
        try {
            ...
            meterRegistry.counter("chaos.events.published",
                    "topic", topic,
                    "event_type", envelope.eventType(),
                    "status", "success")
                    .increment();
            meterRegistry.timer("chaos.events.publish.duration",
                    "topic", topic)
                    .record(durationMs, java.util.concurrent.TimeUnit.MILLISECONDS);
            
            log.info("Published event {} to topic {} partition {} offset {}",
                    envelope.eventId(), topic, metadata.partition(), metadata.offset());
            ...
            log.error("Failed to publish event {} to topic {}: {}",
                    envelope.eventId(), topic, e.getMessage(), e);
